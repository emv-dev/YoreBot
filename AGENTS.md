...
- **Decision:** In the existing manual Windows ritual, finish and verify the
  declared model/runtime downloads first, then back up Windows audit policy,
  enable blocked-connection auditing, and install temporary non-loopback block
...
- **Decision:** Keep the 5.68 GB ritual manual-only. After verifying the exact
  ...
  Constrain the generated batch to at most one terminal call, final-only, while
  retaining the same runtime validation as defense in depth.
